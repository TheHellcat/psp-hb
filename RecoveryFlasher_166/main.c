...
void DoCFWInstall( int PspType, char* sPspType )
{
  ...
  if( TargetFW != 0 )
  {
    ...
    if( strcmp("100.PBP", s) == 0 ) { Restore150 = 1; printf( "\n:] OMG! A sexy 1.0.x FW! You're cool! :-)\n\n\n" ); }
  }
...
int main(int argc, char *argv[])
{
  ...
  // / TOOL mod
  //sprintf( MyEBOOTPath, "ms0:/PSP/GAME/RECOVERY/EBOOT.PBP" );
  //sceIoChdir("ms0:/PSP/GAME/RECOVERY");
...
// TN's 6.20 HEN needs some special treatments and savety checks, check if we're running on it
Is620HEN       = hcRfCheckFor620HEN();
Is620HENf0prot = hcRfCheck620HENf0Prot();

if( (Is620HEN == 1) && (Is620HENf0prot == 1) )
{
	// running RF with f0 protection enabled.... doesn't make much sens (and is dangerous, due to the poor implementation of the "protection")
  printf("\n\n   /!\\ FATAL ERROR:\n\n   The Flash0: protection of 6.20-TN seems to be enabled,\n   Recovery Flasher can not continue!\n\n" );
  printf("   Please disable the flash0 protection from the HEN's VSH menu\n   and run Recovery Flasher again.\n\n\n" );
  printf("   Press any button to return to the XMB...." );
  hcMiscWaitForAnyButton();
  sceKernelExitGame();
}
...
if( Is620HEN == 1 )
{
  // check if plugins are configured while we're on 6.20-TN or 6.35 HEN
  // this is required as the "reboot to updater mode" is broken and does not work on those HENs
  i = 0;
  
  r = sceIoOpen("ms0:/seplugins/game.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ms0:/plugins/game.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ef0:/seplugins/game.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ef0:/plugins/game.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  ...
  if (r > 0)
  {
  	// there are configured game mode plugins
  	// disable (by renaming game.txt), notify user and ask for relaunch
    sceIoRename("ms0:/seplugins/game.txt", "ms0:/seplugins/game_rf-disabled.txt");
    sceIoRename("ms0:/plugins/game.txt", "ms0:/plugins/game_rf-disabled.txt");
    sceIoRename("ef0:/seplugins/game.txt", "ef0:/seplugins/game_rf-disabled.txt");
    sceIoRename("ef0:/plugins/game.txt", "ef0:/plugins/game_rf-disabled.txt");
    printf("\n\n   /!\\ ERROR:\n\n   There were configured game mode plugins!\n\n   For security reasons, Recovery Flasher can not work with plugins\n   running in the background.\n\n" );
    printf("   Recovery Flasher already DISABLED all configured game mode\n   plugins!\n" );
    printf("   Recovery Flasher will now exit back to the XMB, please launch it\n   again!\n" );
    printf("   All disabled plugins will automatically be re-enabled upon the\n   next launch.\n\n\n");
    printf("   Press any button to return to the XMB...." );
    hcMiscWaitForAnyButton();
    sceKernelExitGame();
  }
  
  // check if we previously disabled plugins and re-enable them
  i = 0;
  r = sceIoOpen("ms0:/seplugins/game_rf-disabled.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ms0:/plugins/game_rf-disabled.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ef0:/seplugins/game_rf-disabled.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = sceIoOpen("ef0:/plugins/game_rf-disabled.txt", PSP_O_RDONLY, 0777);
  if( r>0 ) { i = 1; sceIoClose( r ); }
  r = i;
  
  if (r > 0)
  {
    sceIoRename("ms0:/seplugins/game_rf-disabled.txt", "ms0:/seplugins/game.txt");
    sceIoRename("ms0:/plugins/game_rf-disabled.txt", "ms0:/plugins/game.txt");
    sceIoRename("ef0:/seplugins/game_rf-disabled.txt", "ef0:/seplugins/game.txt");
    sceIoRename("ef0:/plugins/game_rf-disabled.txt", "ef0:/plugins/game.txt");
    printf(" [i] plugins re-enabled....\n\n");
  }
}
  ...
  if( argc != 0 )
  {
    ...
    if( *(argv[0]+r) !=  0x32 )
    {
      if( Is620HEN != 1 ){ r = hcRfHelperRebootToUpdater(); }
    }
  } else {
  	if( Is620HEN != 1 ){ r = hcRfHelperRebootToUpdater(); }
  }
  ...
  if( tac >= 0x00720000 ) { PspType = 5000; }
  if( tac >= 0x00810000 ) { PspType = 4000; }
  printf( "\nPSP version: %i\n", PspType );
  ...
  if( PspType == 4000 ) { sprintf( sPspType, "PSP Brite 2" ); }
  if( PspType == 5000 ) { sprintf( sPspType, "PSPgo" ); }
  ...
  {
    ClearScreen();
    PrintNote( "  Even worse: 6.20 or higher HEN detected!\n  Even MORE functionality will be limited!" );
    sceKernelDelayThread( 5000000 );
  }
