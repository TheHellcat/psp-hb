...
int WriteFile(char *file, void *buf, int size)
{
  ...
  // if .PBP is 1.50, files are always to be written (to Fat PSPs), override detection
  if( is150 == 1 )
  {
    ThisFileType = 1000;
  }
  ...
  // check if we are handling a 1.50 .PBP, since that one needs to be extracted to the memstick
  if( is150 == 1 )
  {
    sprintf( tfile, "ms0:/flash_backup/OFW_1.50/f0/%s", file+8 );
    sprintf( file, "%s", tfile+29 );
  }
...
void psarSetPBPPath( char* path )
{
  ...
  fwflag = 0;
  if( strcmp(path, "380.PBP") == 0 ) { fwflag = 1; }
  if( strcmp(path, "390.PBP") == 0 ) { fwflag = 1; }
  if( strcmp(path, "401.PBP") == 0 ) { fwflag = 2; }
  if( strcmp(path, "500.PBP") == 0 ) { fwflag = 3; }
  	
  if( strcmp(path, "150.PBP") == 0 ) { is150 = 1; } else { is150 = 0; }
