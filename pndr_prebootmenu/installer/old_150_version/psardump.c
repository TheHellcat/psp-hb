...
int extract_psar(u8 *dataPSAR, int cbFile, u8 *dataOut, u8 *dataOut2)
{
  ...
  if (cbOut <= 0)
  {
    printf("ERROR: Failed to decode(1)\n");
    return -1;
  }
  if (cbOut != SIZE_A)
  {
    printf("ERROR: Start PSAR chunk has invalid size (%d)\n", cbOut);
    return -1;
  }
  ...
  while (iBase < cbFile-OVERHEAD)
  {
    ...
    if (cbOut <= 0)
    {
      printf("ERROR: Abort return cb=%d [@$%x]\n", cbOut, iBase);
      //break;
    }
    if (cbOut != SIZE_A)
    {
      printf("ERROR: Bad size for filename %d\n", cbOut);
      //break;
    }
      ...
      if (cbOut > 10 && dataOut[0] == 0x78 && dataOut[1] == 0x9C)
      {
        ...
        if (ret == cbExpanded)
        {
          ...
          if (strcmp(name, "flash0:/kd/loadexec.prx") == 0)
          {
            ...
            if (sz < 0)
            {
              printf("Cannot decrypt loadexec.\n");
            } else {
              //WriteFile("ms0:/loadexec.prx", dataOut, sz);
            }
          }
        } else {
          printf("ERROR: deflate error $%x\n", ret);
        }
      } else {
