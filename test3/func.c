...
// information for hidden module extraction of 6.xx updaters
typedef struct {
      u8 VerMajor;
      u8 VerMinor;
      u8 *MD5;
...
int HcUpdaterPrx_NumEntries = 4;
DATAPSP_INFO HcUpdaterPrx_Info[4] = {
...
void pspUpdDecryptXor(u8 *buf, int bufsize, u8 a, u8 b, u8 c)
{
    u8 x = a;
    u8 y = b;
    u8 z = c;
...
{ // *BOTH* buffers must be SAME SIZE and large enough to hold the complete decrypted and decompressed file!!!
  int done;
  int r;
  int sz_out;
  ...
  while( done == 0 )
  {
  	// loop as long as we don't have the plain version of the .PRX
  	
  	// check what we got and perform actions
  	if (!memcmp(buffer_file+1, "ELF", 3))
  	{
  		// yay, we're done, tell the loop the exit
  		done = 1;
		} else if (*(u16 *)&buffer_file[0] == 0x8B1F) {
		  // GZIP compressed
		  r = pspDecompress( buffer_file, buffer_work, buffers_size );
		  if(r<1)
		  {
		    printf( "(!) ERROR decompressing module: 0x%08x\n", r );
		    sz_out = 0;
		    done = 1;
		  } else {
		    memcpy( buffer_file, buffer_work, r );
		    sz_out = r;
		  }
	  } else if (!memcmp(buffer_file, "~PSP", 4)) {
	  	// encrypted
	  	r = pspDecryptPRX(buffer_file, buffer_work, buffers_size);
	  	if(r<1)
		  {
		    printf( "(!) ERROR decrypting module: 0x%08x\n", r );
		    sz_out = 0;
		    done = 1;
		  } else {
		    memcpy( buffer_file, buffer_work, r );
...
void hcExtractUpdaterSepcialPRX( u8* b_dataPSAR, char* pbppath )
{
  ...
  while( 1 )
  {
    ...
    for( i=0; i<dpsp_l; i+=4 )
    {
      ...
      if( m == tag )
      {
        ...
        {
        	// first we copy the .PRX file into the buffer where we loasded the encrypted updater.prx (before we decrypted it, that buffer should be unused now)
        	memcpy( b_dataPSAR, modptr[0], modlen[0] );
        	// then we let the decrypt'n'decompress function roll over it
        	r = hcDecryptDecrompressModule( b_dataPSAR, b_dataPSAR + (modlen[0]*3), modlen[0]*3 );
        	// well, yah, and write it out
          hcWriteFile( "lfatfs_updater.prx", b_dataPSAR, r );
          foundprx = 1;
        }
        if( strcmp( (char*)(modptr[0]+0x0A), "sceNAND_Updater_Driver" ) == 0 )
        {
        	memcpy( b_dataPSAR, modptr[0], modlen[0] );
        	r = hcDecryptDecrompressModule( b_dataPSAR, b_dataPSAR + (modlen[0]*3), modlen[0]*3 );
          hcWriteFile( "nand_updater.prx", b_dataPSAR, r );
          foundprx = 1;
        }
      }
    }
  ...
  for( i=0; i<dpsp_l; i+=4 )
  {
    ...
    if( m == tag )
    {
      ...
      if( strcmp( (char*)(modptr[0]+0x0A), "sceLflashFatfmtUpdater" ) == 0 )
      {
        memcpy( b_dataPSAR, modptr[0], modlen[0] );
        r = hcDecryptDecrompressModule( b_dataPSAR, b_dataPSAR + (modlen[0]*3), modlen[0]*3 );
        hcWriteFile( "lflash_fatfmt_updater.prx", b_dataPSAR, r );
      }
      if( strcmp( (char*)(modptr[0]+0x0A), "IplUpdater" ) == 0 )
      {
        memcpy( b_dataPSAR, modptr[0], modlen[0] );
        r = hcDecryptDecrompressModule( b_dataPSAR, b_dataPSAR + (modlen[0]*3), modlen[0]*3 );
        hcWriteFile( "ipl_update.prx", b_dataPSAR, r );
      }
    }
  }
  ...
  if( foundprx == 1 )
  {
  	// if we already found the hidden modules we're certainly not dealing with a 6.xx updater
  	// so, no need to continue, exit here
  	return;
  }
  
  // 6.xx updater's module extraction
  // ----------------------------------
  
  // cycle through our array of 6.xx updater info
  for( u = 1; u < HcUpdaterPrx_NumEntries; u++)
  {  // yes, we start at "1", skipping the first entry at "0" as that one is for 5.00 updaters already handled by the code above

    // copy the loaded and decrypted DATA.PSP (updater executable) to a temp. workbuffer
    // as we're performing destructive actions , killing the original data but we need the original data a few times
    memcpy( b_dataPSAR, buff_off, dpsp_l );

    // descramble the hidden modules according to the info from our all-knowing array
    pspUpdDecryptXor( b_dataPSAR + HcUpdaterPrx_Info[u].FATFS_OFF, HcUpdaterPrx_Info[u].NAND_OFF - HcUpdaterPrx_Info[u].FATFS_OFF, HcUpdaterPrx_Info[u].CODE1, HcUpdaterPrx_Info[u].CODE2, HcUpdaterPrx_Info[u].CODE3 );
    pspUpdDecryptXor( b_dataPSAR + HcUpdaterPrx_Info[u].NAND_OFF, HcUpdaterPrx_Info[u].FATFMT_OFF - HcUpdaterPrx_Info[u].NAND_OFF, HcUpdaterPrx_Info[u].CODE1, HcUpdaterPrx_Info[u].CODE2, HcUpdaterPrx_Info[u].CODE3 );

    // and save the modules
    tag = 0x5053507E;
    for( i=0; i<dpsp_l; i+=4 )
    {
      m = *(u32*)(b_dataPSAR+i);
      if( m == tag )
      {
        modptr[0] = &b_dataPSAR[i];
        modlen[0] = (*(u32*)(b_dataPSAR+i+0x2C));
        if( strcmp( (char*)(modptr[0]+0x0A), "sceLFatFs_Updater_Driver" ) == 0 )
        {
          memcpy( b_dataPSAR, modptr[0], modlen[0] );
          r = hcDecryptDecrompressModule( b_dataPSAR, b_dataPSAR + (((dpsp_l*2)+2048) & 0xFFFFFF00 ), modlen[0]*3 );
          hcWriteFile( "lfatfs_updater.prx", b_dataPSAR, r );
        }
        if( strcmp( (char*)(modptr[0]+0x0A), "sceNAND_Updater_Driver" ) == 0 )
        {
    ...
    /*
        (i) urks, the "b_dataPSAR + (dpsp_l*2)+2048" to get a workbuffer is REALLY bad coding style,
            kids, don't try this at home!!!
            We assume there's enough usable memory left behind everything we can use as workbuffer....
            Usually this will end in crashes, buserrors and bufferoverflows, in THIS sepcial case
            we just know the buffer we got passed is our huge PSAR buffer and we DO have plenty of
            space there.... but only 'cause we 100% know all that happened before this func' was called.
            I don't like it this way, but it works and was the most easy way to implement the new decoding function into it....
    */
  }
}
...
int hcRfCheckFor620HEN( void )
{
  ...
  //if( (fw == 620) || (fw == 635) ) { r = 1; } else { r = 0; }
  if( fw >= 620 ) { r = 1; } else { r = 0; }
...
int hcRfCheck620HENf0Prot( void )
{
  ...
  // try to re-assign the flash0: device, if it fails we're very likely running with enabled flash0 protection of the 6.20-TN HEN
