...
void hcRfHelperGetMoboVersions( int* baryon, int* tachyon, int* fuseid_high, int* fuseid_low )
{
  ...
  fuse = sceSysreg_driver_4F46EEDE();
  
  pspSdkSetK1( k );
  (*baryon) = by;
  (*tachyon) = ty;
  (*fuseid_high) = *(int*)((int)(&fuse)+4);
  (*fuseid_low)  = *(int*)((int)(&fuse));
}
...
int hcRfHelperRebootToUpdater( void )
{
  ...
  param.argp = "ms0:/PSP/GAME/CFWINSTALL/EBOOT.PBP s2";
  param.args = 38;
  param.key = "updater";
  ...
  param.unk4 = 0;
  param.unk5 = 0x10000;
  r = sceKernelLoadExecVSHMs1( "ms0:/PSP/GAME/CFWINSTALL/EBOOT.PBP", &param );
