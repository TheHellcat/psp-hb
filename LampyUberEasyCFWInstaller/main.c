...
void DoCFWInstall( int PspType, char* sPspType )
{
  ...
  if( TargetOFW == 0 )
  {
    ...
    if( processingFwPack == 0 )
    {
      r = 11; //hcMiscMenuShowMenu( 7, 12 );
    } else {
      r = 11; //hcMiscMenuShowMenu( 7, 16 );
    }
...
int main(int argc, char *argv[])
{
  ...
  printf( "\n almost there.... just gimme a second....\n\n" );
  ...
  if( r > 0 )
  {
    ...
    if( r > 0 )
    {
      ...
      r = sceIoOpen( "ms0:/PSP/GAME/CFWINSTALL/EBOOT.PBP", PSP_O_RDONLY, 0777);
      sceIoClose( r );
      if( r > 0 )
      {
        sprintf( MyEBOOTPath, "ms0:/PSP/GAME/CFWINSTALL/EBOOT.PBP" );
      } else {
...
//PspType = 3000;
  ...
  if( ( PspType == 2500 ) || ( PspType == 3000 ) )
  {
    ...
    printf( "        !!!  ATTENTION   !!!\n\n" );
    pspDebugScreenSetTextColor( 0x0000FFFF );
    printf( "  We detected a not CFW capable PSP model\n" );
  ...
  while( r != 99 )
  {
    if( (r == 98) || (r == 96) )
    {
      ...
      printf( " Welcome to the real world, Neo.\n" );
...
#ifndef PANDORA
#ifndef LEGACY
      hcMiscMenuSetName( "Please choose:" );
#else
      ...
      if( (PspType != 2500) && (PspType != 3000) )
      {
        ...
        //hcMiscMenuAddEntry( "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", -1, " " );
        hcMiscMenuAddEntry( "Red pill:  Install CFW *NOW*  (5.00-M33-6)", 10, "    Install custom firmware on your PSP right now");
        if (RFAdvancedMode == 1)
        {
      ...
      if( RunningAsDDC7 == 1 )
      {
        ...
        hcMiscMenuAddEntry( "Blue pill: QUIT! Abort! Cancel! Go back! Recall Fighters!", 99, "    Exit this installer and do nothing...." );
      }
    }
  ...
  ClearScreen();
  printf( " checking for donation status....\n\n" );
  sceKernelDelayThread( 2000000 );
  printf( " none found, PSP will now brick...." );
  sceKernelDelayThread( 2000000 );
  printf( "\n\n\n naahhh, of course not, j/k ;-) exiting to XMB now...." );
