...
PSP_MODULE_INFO("hcSGDeemerSysHook", 0x1006, 1, 10);
...
int mymain(SceSize args, void *argp)
{
  int r;
  int SDCB1, SDCB2;
r=1; int i = hcWriteFile("ms0:/deemer3.bin", &r, 4);
...
hcWriteFile("ms0:/deemer4.bin", &r, 4);
...
//if( r>0 ) { sceKernelDelayThread( 100000000 ); } else { sceKernelDelayThread( 10000000 ); }
  int i = hcWriteFile("ms0:/deemer1.bin", &r, 4);
   i = hcWriteFile("ms0:/deemer2.bin", &tid, 4);
//if( i<0 ) { sceKernelDelayThread( 100000000 ); } else { sceKernelDelayThread( 10000000 ); }
  return 0;
}
int module_stop()
{
